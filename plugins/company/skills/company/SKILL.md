...
| アフィリエイト運営 | アフィリエイト、アソシエイト、商品紹介、レビュー記事、紹介料 |
...
部署の中の仕事を「係」単位に割ったテンプレート集。
ユーザーが「〇〇を回す係を4人作って」のように**役割単位**で依頼した場合に使う。
...
1. パックの各ファイルを部署の `roles/` にコピーする（Amazonアフィリエイトなら `.company/affiliate/roles/`）
2. ユーザーのジャンル・事業内容に合わせて書き換える
3. 該当パックがない場合は、下の4行フォーマットで新しく作る

### 係ファイルの4行フォーマット

1. **何をする人か** — 担当範囲
2. **渡す材料** — インプット
3. **出してほしい形** — アウトプットの型（表・行数・列まで具体的に）
4. **やったらあかんこと** — NG

**4 の「ジャンル別のNG」は空欄のままユーザーに渡す。**
そのジャンル固有の地雷はこちらでは分からないので、埋めずに「ここは自分で足してください」と伝える。
一般的な規約・法令まわりの共通NGだけこちらで書く。
